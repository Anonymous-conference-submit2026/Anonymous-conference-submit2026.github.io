...
![image-20251112230316435](C:\Users\admin\OneDrive\Rank\论文写作\DPT\ICLR_rebuttal\assets\image-20251112230316435.png)



**Q4：** 若应用于带门控机制（如 SwiGLU）或 Post-Norm 的 ViT 变体，是否仍可直接进行层融合？论文是否验证过这种情况下的有效性？

——确实直接使用 ——把SwiGLU转化为Relu —— Relu strikes back的文章放上去
...
- 更长的50个epoch的情况下，恢复不对称性肯定更明显，因为都会变成无损的（小数据集——表格呈现）
...
1. 精度下降来自于变换的差异， **可以考虑用去除前后的分布差异去证明**

2. 精度恢复得快的原因是——非线性变换相比attention的变换更容易近似？(这个考虑一下怎么回答)
